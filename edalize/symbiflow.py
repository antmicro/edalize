...
class Symbiflow(Edatool):
    ...
    def configure_nextpnr(self):
        ...
        target_family = None
        for family in getattr(self, "nextpnr_families"):
            if family in part:
                target_family = family
                break

        if target_family is not None or arch is not "fpga_interchange":
            logger.error("Couldn't find family for part: {}. Available families: {}".format(part, ", ".join(getattr(self, "families"))))
    ...
    def configure_main(self):
        ...
        elif self.tool_options.get('pnr') == 'vpr':
            self.configure_vpr()
        else:
