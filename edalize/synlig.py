...
class Synlig(Edatool):
    ...
    def _configure_readsystemverilog(self):
        """returns a list of Synlig commands to process Verilog and SystemVerilog sources"""
        unused_files = []
        file_table = []
        includes = set()
    ...
    def gen_script_nosynth(self, read_command, plugins):
        """Generates a TCL script for Synlig to parse SystemVerilog files without synthesis"""
        ...
        tcl_script = "synlig_nosynth.tcl"
        self.render_template("synlig_nosynth.tcl.j2", tcl_script, template_vars)
        commands.add(
            ["synlig", "-l", "synlig.log", "-p", f"'tcl {tcl_script}'"],
            [rtlil],
            [],
            [tcl_script],
        )
    ...
    def gen_script(self, file_table, incdirs, plugins, commands):
        ...
        self.render_template(
            "edalize_synlig_procs.tcl.j2", "edalize_synlig_procs.tcl", template_vars
        )

        if not synlig_template:
            self.render_template(
                "synlig-script-tcl.j2", "edalize_synlig_template.tcl", template_vars
            )

        commands.add(
            ["synlig", "-l", "synlig.log", "-p", f"'tcl {template}'"],
            [default_target],
