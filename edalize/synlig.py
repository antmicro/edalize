...
class Synlig(Edatool):
        ...
        if api_ver == 0:
            options = {
                    'members' : [
                        ...
                        {'name' : 'synlig_as_subtool',
                         'type' : 'bool',
                         'desc' : 'Determines if Synlig is run as a part of bigger toolchain, or as a standalone tool'},
                        {'name' : 'makefile_name',
                         'type' : 'String',
                         'desc' : 'Generated makefile name, defaults to $name.mk'},
                        {'name' : 'synlig_template',
                         'type' : 'String',
                    ...
                    'lists' : [
                        {'name' : 'synlig_read_options',
                         'type' : 'String',
                         'desc' : 'Addtional options for the read_* command (e.g. read_verlog or read_uhdm)'},
                        {'name' : 'synlig_synth_options',
                         'type' : 'String',
                         'desc' : 'Additional options for the synth command'},
                        {'name' : 'synlig_extra_passes',
                         'type' : 'String',
                         'desc' : 'Additional synlig passes executed after synth'},
                        ]
                    }
    ...
    def configure_main(self):
        # write Synlig tcl script file

        synlig_template = self.tool_options.get('synlig_template')
        synlig_read_options = " ".join(self.tool_options.get('synlig_read_options', []))
        ...
        if "frontend=surelog" in synlig_synth_options:
            ...
            synlig_synth_options.remove("frontend=surelog")
            ...
            if "file_type" in f:
                ...
                if cmd:
                    if not self._add_include_dir(f, incdirs):
                        file_table.append(cmd + synlig_read_options + ' {' + f['name'] + '}')
                else:
        ...
        template = synlig_template or 'edalize_synlig_template.tcl'
        template_vars = {
                ...
                'synth_options'       : " ".join(self.tool_options.get('synlig_synth_options', '')),
                'write_command'       : "write_" + output_format,
                'default_target'      : output_format,
                'edif_opts'           : '-pvector bra' if arch=='xilinx' else '',
                'synlig_template'      : template,
                'name'                : self.name,
                'plugins'             : "plugin -i %s \n"*len(plugins) % tuple(plugins),
                'extra_passes'        : "\n".join(self.tool_options.get('synlig_extra_passes', '')),
        }
        self.render_template('edalize_synlig_procs.tcl.j2',
                             'edalize_synlig_procs.tcl',
                             template_vars)

        if not synlig_template:
            self.render_template('synlig-script-tcl.j2',
                                 'edalize_synlig_template.tcl',
                                 template_vars)

        commands.add(['synlig', '-l', 'synlig.log', '-p', f'"tcl {template}"'],
                         [f'{self.name}.{output}' for output in ['blif', 'json','edif']],
                         [template] + additional_deps)
        if self.tool_options.get('synlig_as_subtool'):
